...
def scan_node(node, buf, level=0, map_name=None):
    global reg, model_texture
    for subnode in node:
        ...
        if subnode[0] == "OBJECT":
            ...
            texture_secondary = obj.get("objsectxtr", '').lower()
            if texture_secondary and texture_secondary != 'default0':
                texture += '|' + texture_secondary
            coefs = obj.get("objcomplection", None)
            parts = obj.get("objbodyparts", None)
            if parts:
                parts = ':'.join(parts)
            else:
                parts = 'null'
        ...
        if type(subnode[1]) == str:
            ...
            buf[0] += "\n"
            scan_node(subnode[1], buf, level + 1, map_name=map_name)
        else:
            print("WTF")
